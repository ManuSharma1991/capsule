...
export function errorHandler(
    ...
    switch (err.code) {
        case 'SQLITE_CONSTRAINT_UNIQUE':
            ...
            if (err.message && err.message.toLowerCase().includes('users.empid')) { // Example check
                uniqueErrorCode = AUTH_ERROR_CODES.USER_ALREADY_EXISTS_EMP_ID;
                uniqueMessage = 'An employee with this ID already exists.';
            } else if (err.message && err.message.toLowerCase().includes('users.email')) { // Example check
