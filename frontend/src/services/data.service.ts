...
import type { Case, CauselistCase } from '../types/cases'; // Import Case type
...
const exportCauselist = (
  causelistCases: CauselistCase[]
): Promise<AxiosResponse<{ message: string; filepath: string }>> => {
  return axiosInstance.post('/reports/causelist', { causelistCases });
};
