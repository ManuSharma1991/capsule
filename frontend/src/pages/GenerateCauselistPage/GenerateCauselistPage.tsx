...
const GenerateCauselistPage: FC = () => {
  ...
  const handleSaveEditedCase = () => {
    if (editedCase && editingCaseId) {
      setCauselistCases((prevCases) =>
        prevCases.map((c) =>
          c.id === editingCaseId ? ({ ...c, ...editedCase } as CauselistCase) : c
        )
      );
  ...
  const groupedCases: GroupedCauselistCases = causelistCases.reduce(
    (acc: GroupedCauselistCases, caseItem) => {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Grid
        ...
        spacing={2}
        alignItems="center"
        sx={{ mb: 3, backgroundColor: '#f5f5f5', p: 2, borderRadius: 1 }}
      >
        <Grid
          size={{ xs: 12, sm: 4 }}
          sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', gap: 1 }}
        >
          <Grid size={{ xs: 9 }}>
          ...
          <Grid size={{ xs: 3 }}>
            <Button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    'Are you sure you want to reset the date? All unsaved data will be lost.'
                  )
                ) {
                  setSelectedDate(null);
                  setCauselistCases([]);
                      ...
                      <TableRow key={caseItem.id}>
                        {editingCaseId === caseItem.id ? (
                          <>
                            ...
                            <TableCell>
                              <TextField
                                name="appellantName"
                                value={
                                  editedCase?.filedBy === 'A'
                                    ? editedCase.appellantName
                                    : editedCase?.respondantName || ''
                                }
                                onChange={handleEditChange}
                                size="small"
                              />
                            ...
                            <TableCell>
                              <Button onClick={handleSaveEditedCase} size="small">
                                Save
                              </Button>
                          ...
                          <>
                            ...
                            <TableCell width={200}>
                              {caseItem?.filedBy === 'ASSESSEE'
                                ? caseItem.appellantName
                                : caseItem?.respondantName}
                            </TableCell>
